...
const SalesInterface = () => {
  ...
  const [saleForPrinting, setSaleForPrinting] = useState<{
  ...
  const {
    data: settings = { shop_name: "Pos Flow", shop_address: "العنوان هنا" },
  } = useQuery({
    queryKey: ["settings"],
          ...
          <Card className="bg-white/40 backdrop-blur-md border-white/40 shadow-xl shadow-blue-900/5 overflow-hidden group">
            ...
            <CardContent className="relative">
              <form onSubmit={handleBarcodeSubmit} className="flex gap-3">
                ...
                <Button
                  type="submit"
                  className="h-14 px-8 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/20 active:scale-95 transition-all"
                >
                  إضافة سريعة
                </Button>
              </form>
              ...
              {products.length === 0 ? (
                <div className="text-center py-20 flex flex-col items-center">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    قم بإضافة منتجات من علامة تبويب المنتجات
                  </p>
                </div>
              ) : (
                  ...
                  <>
                    <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                      {cart.map((item) => (
                        <div
                          key={item.id}
                          className="group relative flex flex-col p-4 bg-white/50 backdrop-blur-sm border border-blue-100/50 rounded-2xl gap-3 transition-all hover:bg-white hover:shadow-lg hover:shadow-blue-500/5"
                        >
      ...
      <AlertDialog open={isPrintDialogOpen} onOpenChange={setIsPrintDialogOpen}>
        <AlertDialogContent dir="rtl">
          ...
          <AlertDialogFooter className="flex-row-reverse gap-2">
            ...
            <AlertDialogCancel onClick={() => setIsPrintDialogOpen(false)}>
              إغلاق
            </AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
        ...
        <div className="printable-invoice">
          ...
          <div className="space-y-1 border-b border-dashed pb-2 mb-4">
            <div className="flex justify-between font-bold">
              ...
              <span className="font-mono">
                EGP {saleForPrinting.total.toFixed(2)}
              </span>
            </div>
          ...
          <div className="text-center space-y-1">
            <p className="font-bold">
              Products Count{" "}
              {saleForPrinting.items.reduce(
                (acc, item) => acc + item.quantity,
                0,
              )}
            </p>
            <p className="text-sm italic mt-4">شكراً لزيارتكم!</p>
            <p className="text-[10px] text-gray-400 mt-4 tracking-tighter uppercase">
              Powered by Pos Flow
            </p>
          </div>
        </div>
